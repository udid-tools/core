...
## Verify version 0.1.0-beta.6

Download the immutable release assets:

```bash
gh release download v0.1.0-beta.6 --repo udid-tools/core --dir core-release
cd core-release
```
...
```bash
sha256sum --check udid-tools-core-0.1.0-beta.6.tgz.sha256
```

On macOS, use the compatible checksum command:

```bash
shasum --algorithm 256 --check udid-tools-core-0.1.0-beta.6.tgz.sha256
```

Verify the tarball's keyless Sigstore signature, certificate identity, and transparency-log proof:

```bash
cosign verify-blob udid-tools-core-0.1.0-beta.6.tgz \
  --bundle udid-tools-core-0.1.0-beta.6.tgz.sigstore.json \
  --certificate-identity \
  "https://github.com/udid-tools/core/.github/workflows/release.yml@refs/tags/v0.1.0-beta.6" \
...
```bash
gh attestation verify udid-tools-core-0.1.0-beta.6.tgz \
